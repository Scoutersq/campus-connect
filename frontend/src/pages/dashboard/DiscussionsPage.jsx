  ...
  return (
    <div className="flex h-full flex-col gap-6">
      ...
      <div className="grid flex-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        <section className="flex max-h-full flex-col gap-4 overflow-hidden rounded-3xl border border-orange-100 bg-white/80 p-6 shadow-sm">
        ...
        <div className="hidden min-h-0 lg:flex">
          {selectedDiscussion ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-3xl border border-dashed border-orange-200 bg-white/60 p-10 text-center text-sm text-slate-500">
              <p className="max-w-sm">
                Choose a discussion to view its live chat and join the conversation.
              </p>
            </div>
          )}
        </div>
      ...
      {selectedDiscussion && (
        <div className="lg:hidden">
          <LiveDiscussionPanel
            key={`mobile-${selectedDiscussion.id}`}
            discussion={selectedDiscussion}
            onClose={() => setSelectedId(null)}
            onMembershipChange={handleMembershipChange}
          />
